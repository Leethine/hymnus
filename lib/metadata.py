...
class SQLiteMetadataReader(metaclass=SingletonMeta):
  ...
  def getPartialComposers(self, items_per_page: int, page_number: int, listed_only=True) -> list:
    ...
    if listed_only:
      rows = SQLite3Adapter().selectPartialRows("SELECT * FROM Composers WHERE listed != 0 ORDER BY code;", \
                                            items_per_page, page_number)
    else:
      rows = SQLite3Adapter().selectPartialRows("SELECT * FROM Composers ORDER BY code;", \
                                            items_per_page, page_number)
    return rows
  ...
  def getPartialPieces(self, items_per_page: int, page_number: int) -> list:
    ...
    rows = SQLite3Adapter().selectPartialRows("SELECT * FROM Pieces ORDER BY title;",
                                         items_per_page, page_number)
    return rows
  ...
  def searchPiecesBySubstr(self, keyword: str) -> list:
    ...
    rows = SQLite3Adapter().selectRows(f"SELECT * FROM Piece_search WHERE context LIKE '%{keyword}%' ORDER BY context;")
    for r in rows:
      if 'folder_hash' in r:
        piece_metadata = SQLite3Adapter().selectRows(f"SELECT * FROM Pieces WHERE folder_hash = '{r['folder_hash']}';")
        if piece_metadata:
  ...
  def getComposerPieces(self, composer_code: str) -> list:
    ...
    rows = SQLite3Adapter().selectRows(f"SELECT * FROM Pieces WHERE composer_code = '{composer_code}' OR arranger_code = '{composer_code}' ORDER BY title;")
    return rows
  ...
  def getComposerPartialPieces(self, composer_code: str, items_per_page: int, page_number: int) -> list:
    ...
    rows = SQLite3Adapter().selectPartialRows(f"SELECT * FROM Pieces WHERE composer_code = '{composer_code}' OR arranger_code = '{composer_code}' ORDER BY title;", \
                                         items_per_page, page_number)
    return rows
  ...
  def getPartialCollections(self, items_per_page: int, page_number: int) -> list:
    ...
    rows = SQLite3Adapter().selectPartialRows("SELECT * FROM Collections ORDER BY title;",
                                         items_per_page, page_number)
    return rows
  ...
  def getCollectionPieces(self, collection_code: str) -> list:
    ...
    rows = SQLite3Adapter().selectRows(f"SELECT * FROM Pieces WHERE collection_code LIKE '%{collection_code}%' ORDER BY title;")
    return rows


  def getComposerCollections(self, composer_code: str) -> list:
    """Get collections related to a specific composer."""
    rows = SQLite3Adapter().selectRows(f"SELECT * FROM Collections WHERE composer_code = '{composer_code}' ORDER BY title;")
    return rows
  ...
  def countComposerCollections(self, composer_code: str) -> int:
    ...
    count = SQLite3Adapter().countRows(f"SELECT COUNT(*) FROM Collections WHERE composer_code = '{composer_code}';")
    return count
  ...
  def collectionHasPiece(self, collection_code: str, piece_hash: str) -> bool:
    ...
    count = SQLite3Adapter().countRows(f"SELECT COUNT(*) FROM Pieces WHERE collection_code LIKE '%{collection_code}%' \
...
class SQLiteWriteMetadata(metaclass=SingletonMeta):
  ...
  def createComposer(self, firstname: str, lastname: str, knownas_name: str, bornyear=-1, diedyear=-1) -> str:
    ...
    err = SQLite3Adapter().updateRows(f"INSERT INTO Composers (code, firstname, lastname, knownas_name, bornyear, diedyear) \
  ...
  def createPiece(self, composer_code: str, title: str, subtitle="", subsubtitle="", opus="", dedicated="", \
    ...
    if self.__checkComposerExist(composer_code):
      err = SQLite3Adapter().updateRows(f"INSERT INTO Pieces \
      ...
      composer = SQLite3Adapter().selectRows(f"SELECT * FROM Composers WHERE code = '{composer_code}';")[0]
      ...
      err += SQLite3Adapter().updateRows(f"INSERT INTO Piece_Search \
  ...
  def createCollection(self, title: str, subtitle="", subsubtitle="", editor="", \
    ...
    err = SQLite3Adapter().updateRows(f"INSERT INTO Collections \
  ...
  def updateComposer(self, composer_code: str, firstname="", lastname="", \
    ...
    err = SQLite3Adapter().updateRows(f"UPDATE Composers SET \
  ...
  def updatePiece(self, piece_hash: str, title="", subtitle="", subsubtitle="", opus="", \
    ...
    try:
      composer_code = SQLite3Adapter().selectRows(f"SELECT composer_code FROM Pieces WHERE folder_hash = '{piece_hash}';")[0]['composer_code']
      composer = SQLite3Adapter().selectRows(f"SELECT * FROM Composers WHERE code = '{composer_code}';")[0]
      composerfullname = composer['firstname'] + " "+ composer['lastname']
      author = composerfullname + " " + collection_code
      context = f"{title} {subtitle} {subsubtitle} {dedicated} {composerfullname} {collection_code}"
      err = SQLite3Adapter().updateRows(f"UPDATE Piece_Search SET \
    ...
    # update Pieces table
    err += SQLite3Adapter().updateRows(f"UPDATE Pieces SET \
  ...
  def updateCollection(self, collection_code: str, title="", subtitle="", subsubtitle="", \
    ...
    err = SQLite3Adapter().updateRows(f"UPDATE Collections SET \
      title = COALESCE('{title}', title), \
      subtitle = COALESCE('{subtitle}', subtitle), \
      subsubtitle = COALESCE('{subsubtitle}', subsubtitle), \
  ...
  def deleteCollection(self, collection_code: str) -> str:
    ...
    for piece in SQLite3Adapter().selectRows(f"SELECT * FROM Pieces WHERE collection_code LIKE '%{collection_code}%'"):
      ...
      if coll and folder_hash:
        ...
        err += SQLite3Adapter().updateRows(f"UPDATE Pieces SET collection_code = '{new_coll}' \
                                      WHERE folder_hash = '{folder_hash}';")
        err += SQLite3Adapter().updateRows(f"UPDATE Piece_search SET collection_code = '{new_coll}' \
                                      WHERE folder_hash = '{folder_hash}';")
  ...
  def addPieceToCollection(self, piece_hash: str, collection_code: str) -> str:
    ...
    piece_belongingto = SQLite3Adapter().selectRows(f"SELECT collection_code FROM Pieces WHERE folder_hash = '{piece_hash}';")
    ...
    err = SQLite3Adapter().updateRows(f"UPDATE Pieces SET collection_code = '{new_collection_code}' \
                                   WHERE folder_hash = '{piece_hash}';")
    # update Piece_Search table
    err += SQLite3Adapter().updateRows(f"UPDATE Piece_Search SET collection_code = '{new_collection_code}' \
                                    WHERE folder_hash = '{piece_hash}';")
    return err
  ...
  def addComposerImslpLink(self, composer_code: str, imslp_link: str) -> str:
    ...
    err = SQLite3Adapter().updateRows(f"UPDATE Composers SET imslp_url = '{imslp_link}' \
                                   WHERE code = '{composer_code}';")
    return err
  ...
  def addComposerWikiLink(self, composer_code: str, wiki_link: str) -> str:
    ...
    err = SQLite3Adapter().updateRows(f"UPDATE Composers SET wikipedia_url = '{wiki_link}' \
                                   WHERE code = '{composer_code}';")
    return err
