...
class SQLiteReadMetadata(metaclass=SingletonMeta):
  ...
  def getComposerPieces(self, composer_code: str) -> list:
    ...
    rows = DB_SQLITE().selectRows(f"SELECT * FROM Pieces WHERE composer_code = '{composer_code}' OR arranger_code = '{composer_code}' ORDER BY title;")
    return rows
  ...
  def getComposerPartialPieces(self, composer_code: str, items_per_page: int, page_number: int) -> list:
    ...
    rows = DB_SQLITE().selectPartialRows(f"SELECT * FROM Pieces WHERE composer_code = '{composer_code}' OR arranger_code = '{composer_code}' ORDER BY title;", \
                                         items_per_page, page_number)
    return rows
